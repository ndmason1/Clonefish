...
unsigned long P[18];        // P-array
unsigned long S[4][256];    // S-array

using namespace std;

void init(unsigned short *key, int keyLen);
void encryptFile(ifstream* inFile, ofstream* outFile);
void decryptFile(ifstream* inFile, ofstream* outFile);
void init();
...
int main(int argc, char** argv)
{
    ...
    if (argc > 1)
    {
        if (strcmp(argv[1], "-e") == 0 || strcmp(argv[1], "-E") == 0)
        {
            ...
            cout << "name of file passed as argument: " << argv[2] << endl; //std::string(argv[1]);
            ...
            // do encryption with hardcoded key
	    ...
	    else if (strcmp(argv[1], "-d") == 0 || strcmp(argv[1], "-D") == 0)
	    {
            ...
            unsigned pos = outFileName.find(".cfe");
            outFileName = outFileName.substr(0,pos);
            outFileName.append(".plain");
            ...
            // do decryption with hardcoded key
    ...
    else cout << "please specify a valid option" << endl;
    return 0;
}

void encryptFile(ifstream *inFile, ofstream *outFile)
{
	char *key = (char *)"e93d5a68948140f7";
	int keylen = strlen(key);	
	init((unsigned short *)key, keylen);
	
	/*

	//char *plaintext=(char *)"The NetworkCommunication subsystem is responsible for providing network communicating "
	                        "services for RequestProcessing subsystem and ClientSessionManagement subsystem of TAEval "
	                        "system. The ClientSessionManagement subsystem can send client requests to RequestProcessing "
	                        "subsystem and get responses back via TCP sockets.";
	...
	while (plaintext_length > 0) 
	{
		...
		//the first half of the current block
		for (block_length = 0; block_length < 4; block_length++) 
		{
			message_left = message_left << 8;
			if (plaintext_length) 
			{
				message_left += *plaintext++;
				plaintext_length--;
			}
			else 
				message_left += 0;
		...
		//encrypt
		encryptBlock(&message_left, &message_right);
		cout<<message_left<<message_right<<endl;
		ciphertext[i++] = (unsigned char)(message_left >> 24);
		ciphertext[i++] = (unsigned char)(message_left >> 16);
		ciphertext[i++]= (unsigned char)(message_left >> 8);
		ciphertext[i++]= (unsigned char)message_left;
		ciphertext[i++]= (unsigned char)(message_right >> 24);
		ciphertext[i++]= (unsigned char)(message_right >> 16);
		ciphertext[i++]= (unsigned char)(message_right >> 8);
		ciphertext[i++]= (unsigned char)message_right;
		ciphertext_length += 8;
	}
	...
	while(ciphertext_length) 
	{
		...
		{
			message_right = message_right << 8;
			message_right += ciphertext[i++];
			if (ciphertext_length)
			ciphertext_length--;
		}
		decryptBlock(&message_left, &message_right);
		cout<<(char)(message_left >> 24)<<(char)(message_left >> 16)<<(char)(message_left >> 8)<<(char)(message_left)<<(char)(message_right >> 24)<<(char)(message_right >> 16)<<(char)(message_right >> 8)<<(char)(message_right);
	}
	*/
	...
	while (fSize > 0) // encrypt blocks until the file has been read entirely
	{
	    ...
	    if (fSize < BLOCK_SIZE)  // file length is not a multiple of the block size; need to add zero-padding
	    {	        
	        cout << "padding final block..." << endl;
	        unsigned long padBytes = BLOCK_SIZE - fSize;
	        if (fSize < sizeof(long))
	        {	
	            lblock = rblock = 0UL;            
	        ...
	        // add extra block containing padding length
	        lblock = 0UL;
            rblock = padBytes;
            outFile->write( (char*)&lblock, sizeof(long));
            outFile->write( (char*)&rblock, sizeof(long));
	    ...
	    cout << "block read: " << hex << lblock << " " << rblock << endl;
	    
	    encryptBlock(&lblock, &rblock);
	    cout << "block encrypted: " << hex << lblock << " " << rblock << endl;
...
void decryptFile(ifstream *inFile, ofstream *outFile)
{
	...
	while (fSize > 0) // encrypt blocks until the file has been read entirely
	{
	    ...
	    unsigned long rblock;
	    /*
	    if (fSize < 2 * sizeof(long))  // file length is not a multiple of the block size; need to add zero-padding
	    {	        
	        cout << "padding final block..." << endl;
	        unsigned long padBytes = (2 * sizeof(long)) - fSize;
	        if (fSize < sizeof(long))
	        {	            
	            inFile->read(reinterpret_cast< char* >(&lblock),fSize);
	            lblock |= 0UL;
	    ...
	    cout << "block decrypted: " << hex << lblock << " " << rblock << endl; // cout.unsetf ( hex )
...
void init(unsigned short *key, int keyLen) 
{
	...
	for (i=0; i<4; i++) 
	{
		 for (j = 0; j < 256; j++) S[i][j] = S_INIT[i][j];
	}
	j = 0;
	for (i = 0; i < N+2; ++i) 
	{
